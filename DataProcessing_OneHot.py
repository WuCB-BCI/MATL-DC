...
def get_one_subject_dataset(test_id, session):
    ...
    #print(test_id)
    path = 'H:\\Feature_class_P_Research\\de_feature\\feature_for_net_session' + str(session) + '_LDS_de'
    os.chdir(path)
    ...
    # ----------tsne----------
    # np.random.seed(21)
    # idx = np.random.randint(0, 3394, size=100)
    # return feature[idx], one_hot_label_mat[idx]
...
def get_one_subject_dataset_IV(test_id, session):
    ...
    Returns:
        featres:  nparray(3394,310)
        labels:  nparray(3394,3)   one-hot标
    """

    path = 'H:\\Feature_class_P_Research\\de_feature\\feature_for_net_session' + str(session) + '_LDS_de_IV'
    os.chdir(path)
    min_max_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))
...
def get_one_subject_dataset_V(test_id, session):
    ...
    path = 'H:\\Feature_class_P_Research\\de_feature\\feature_for_net_session' + str(session) + '_LDS_de_V'
    os.chdir(path)
    min_max_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))
    info = os.listdir(path)[test_id - 1]
...
def get_one_subject_dataset_all(test_id, class_num):
    """
    Read data from a specified session for a specified subject
